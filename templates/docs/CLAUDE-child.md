...
**Option 1: Relative Path (Recommended - Auto-configured)**
```json
{
  "mcpServers": {
    "norchain-dashboard": {
      "command": "node",
      "args": ["../Master/dashboard/mcp-server/dist/index.js"],
      "env": {
...
**Option 2: GitHub Package (Manual install)**
```bash
# Add to your ~/.npmrc
@norchainofficial:registry=https://npm.pkg.github.com

# Then install globally
npm install -g @norchainofficial/dashboard-mcp
```
...
```json
{
  "mcpServers": {
    "norchain-dashboard": {
      "command": "norchain-dashboard-mcp",
      "env": {
        "SUPABASE_URL": "${env:SUPABASE_URL}",
        "SUPABASE_SERVICE_ROLE_KEY": "${env:SUPABASE_SERVICE_ROLE_KEY}"
      }
...
**Option 3: Clone Master repo alongside**
```
/your/dev/path/
...
**Configuration** is in `.claude/settings.local.json` (auto-generated by sync workflow).
